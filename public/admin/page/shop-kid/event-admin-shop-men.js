$(function() {
  ...
  $("#data-table").on("click", ".deleteProductKid", function() {
    let id = $(this).data("id");
    $.ajax({
      url: "http://localhost:3001/admin/page/shop-kid/" + id,
      method: "GET"
    }).done(function(result) {
      $("#modal-delete").attr(
        "action",
        "http://localhost:3001/admin/page/shop-kid/delete-product-kid/" +
          result.data[0]._id
      );
    });
  });
  $("#data-table").on("click", ".editProductKid", function() {
    let id = $(this).data("id");
    $.ajax({
      url: "http://localhost:3001/admin/page/shop-kid/" + id,
      method: "GET"
      ...
      if ($("#ModelEditProductKid")) {
        ...
        $("#modal-edit").attr(
          "action",
          "http://localhost:3001/admin/page/shop-men/edit-product-kid/" +
            updateData.id
        );
      }
    });
